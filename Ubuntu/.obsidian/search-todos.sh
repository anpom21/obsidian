...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --case)
      ...
      ;;
    --todo)
      REQUIRE_TODO_START=1
      shift
      ;;
    --help)
      echo "Usage: $0 [--tag TAG1,TAG2,...] [--note NOTE1,NOTE2,...] [--string TEXT] [--case] [--todo] [--vault VAULT_NAME]"
      echo ""
      echo "Options:"
      ...
      echo "  --todo                  Require TODO marker at line start (- [ ])"
      ...
      echo "  $0 --todo                                    # Only TODOs at line start"
...
if [ $REQUIRE_TODO_START -eq 1 ]; then
  TODO_RG_FLAGS=""
  TODO_PATTERN="^- \\[ \\]"
else
  TODO_RG_FLAGS=""
...
printf "%s\n" "$matching_files" | xargs -d '\n' -r rg -n --with-filename ${RG_CASE_FLAGS} ${TODO_RG_FLAGS} -- "$TODO_PATTERN" | while IFS= read -r line; do
